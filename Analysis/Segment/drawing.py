...
def drawline(frame, list, joint1, joint2):
    ...
    if(list[joint1][3]=='inferred' or list[joint2][3]=='inferred'):
        frame = cv2.line(frame, (int(round(a)),int(round(b))),(int(round(c)),int(round(d))), (0,255,0),2)
    else:
        frame = cv2.line(frame, (int(round(a)),int(round(b))),(int(round(c)),int(round(d))), (0,0,255),2)
...
def drawframe(list):
    ...
    for _, item in list.items():
        ...
        if(a==float('inf')or b==float('inf') or a==float('-inf')or b==float('-inf')):
            return
        frame = cv2.circle(frame, (int(round(a)),int(round(b))), int(round(rad)), (cval,cval,cval), -1)
